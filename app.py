...
def index():
    """主页 - 返回聊天界面"""
    app_logger.info('收到请求: GET / (主页)')
    app_logger.debug('渲染 index.html 模板')
...
def test_stream():
    ...
    def generate():
        for i in range(5):
            yield f"data: {{'chunk': '测试数据 {i}'}}\n\n"
            time.sleep(0.5)
        yield f"data: {{'done': true}}\n\n"
    return Response(generate(), mimetype='text/event-stream')


@app.route('/api/course/stream', methods=['POST'])
def course_stream():
    """课程大纲生成接口（流式SSE）"""
    request_logger.info('=' * 50)
    request_logger.info('[COURSE] 收到课程大纲生成请求（流式）')

    data = request.json or {}
    topic = data.get('topic', data.get('message', '')).strip()

    if not topic:
        return jsonify({'error': '主题不能为空'}), 400

    request_logger.info(f'[COURSE] topic: {topic}')

    CourseGenerator.set_api_key(API_KEY)
...
@app.route('/api/memory/save-course', methods=['POST'])
def save_course_memory():
    ...
    memory.upgrade_to_long_term(content)
    return jsonify({'result': f'课程"{title}"已保存到记忆'})
...
def generate_ppt():
    ...
    if not slides:
        return jsonify({'error': 'slides不能为空'}), 400

    try:
        generator = PPTGenerator(output_dir='generated_ppt')
        output_path = generator.create_ppt(title, slides, theme)
        filename = os.path.basename(output_path)
        download_url = f'/api/ppt/download/{filename}'
        return jsonify({'success': True, 'file': filename, 'download_url': download_url, 'path': output_path})
    except Exception as e:
        request_logger.error(f'[PPT] 生成失败: {e}')
...
def download_ppt(filename):
    ...
    file_path = os.path.join(ppt_dir, safe_filename)
...
def list_ppts():
    ...
    files = [f for f in os.listdir(ppt_dir) if f.endswith('.pptx') and not f.startswith('~$')]
    files.sort(key=lambda f: os.path.getmtime(os.path.join(ppt_dir, f)), reverse=True)
...
def list_drafts():
    """列出所有草稿"""
    return jsonify({'drafts': drafts_manager.list()})


@app.route('/api/drafts/<draft_id>', methods=['GET'])
def get_draft(draft_id):
    """获取单个草稿"""
    draft = drafts_manager.get(draft_id)
    if not draft:
...
def save_draft():
    """保存草稿"""
    data = request.json or {}
    draft = drafts_manager.save(data)
    return jsonify(draft)
...
def delete_draft(draft_id):
    """删除草稿"""
    if drafts_manager.delete(draft_id):
...
def delete_file():
    ...
    # 安全检查：确保文件在允许的目录中
    allowed_dirs = ['generated_ppt', 'generated_lectures', 'generated_content']
...
if __name__ == '__main__':
    app_logger.info('=' * 60)
    app_logger.info('🎓 MiniMax Agent Web 服务启动中...')
    app_logger.info('=' * 60)
    app_logger.info(f'📅 启动时间: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    app_logger.info('🌐 访问地址: http://127.0.0.1:5000')
    app_logger.info('📋 调试模式: True')
    app_logger.info('=' * 60)
